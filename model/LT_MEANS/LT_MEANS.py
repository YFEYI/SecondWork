...
from my_code.model.LTimeMeans.layers import TransformerBlock, PositionalEmbedding, CrossAttnTRMBlock, PredictHeadALL,InputProjection,Tokenizer

class LTimeMeans(nn.Module):
    def __init__(self, args, n_cluster=0):
        super(LTimeMeans, self).__init__()
        ...
        self.predict_head_all = PredictHeadALL(self.n_clusters,args.data_shape[0],args.num_class, args.dropout)
    ...
    def train_forward(self, x,clusters):
        #x = self.norm(x)
        #输出 ：x:Batch, S, d_model}、h_n:{1, d_model}
        x= self.input_projection(x)     #{B,series,d_model}
        x = x.contiguous()
        # 使用x进一步丰富了词向量的长度后，在词向量维度上选取最大值 最后形状{bs,length}
        if clusters != None:
            tokens = self.tokenizer(x,clusters)
        # 输入Embedding Z=Z+P
        position = self.position(x)
        x = x + position
        # mask_token是自己创建的可训练参数，大小为{d_model, 1}->{bs, S, d_model} 最后混入位置参数
        ...
        # 解耦模块仅对不可见位置的Embedding进行预测,同时保持可见位置的Embedding未更新。
        rep_mask_prediction = self.reg(rep_visible, rep_mask_token)  #{16, 254*radio, d_model}
        token_prediction_prob = self.get_cluster_result(rep_mask_prediction,clusters)
        return [rep_mask, rep_mask_prediction], [token_prediction_prob, tokens]

    def forward(self, x,clusters):
        if self.train_clusters==True:
            ...
            x = self.get_cluster_result(x,clusters)
    ...
    def get_cluster_result(self,x,clusters):
        x_cpu = x.view(-1, x.shape[2]).cpu().detach().numpy()
        x_cpu = torch.from_numpy(clusters.transform(x_cpu)).float()
        cluster_result = x_cpu.reshape(x.shape[0],x.shape[1], -1).to(self.device)
        return cluster_result.squeeze()
