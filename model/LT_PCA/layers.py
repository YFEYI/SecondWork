...
class Tokenizer(nn.Module):
    def __init__(self,device,ipca):
        super(Tokenizer, self).__init__()
        self.device = device
        self.iPCA = ipca

    def forward(self,x, epoches):
        bs, length, dim = x.shape
        x = x.view(-1, dim)
        if epoches < 10:
            self.iPCA.partial_fit(x.cpu().detach().numpy())  # {bs*length, vocab_size}
            ipca_results = self.iPCA.transform(x.cpu().detach().numpy())
        else:
            ipca_results = self.iPCA.transform(x.cpu().detach().numpy())
        ipca_results = torch.from_numpy(ipca_results).float().to(self.device)
        # {bs,length} 内容是最大值的下标
        indexes = ipca_results.max(-1, keepdim=True)[1]
